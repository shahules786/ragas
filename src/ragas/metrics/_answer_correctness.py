...
class AnswerCorrectness(MetricWithLLM, MetricWithEmbeddings):
    ...
    async def _ascore(self, row: t.Dict, callbacks: Callbacks, is_async: bool) -> float:
        ...
        for item in ["answer", "ground_truth"]:
            p_value = self._create_statements_prompt(q, row[item])
        ...
        if self.reproducibility > 1:
            result_text_list = [
                is_statement_present.generations[0][i].text
                for i in range(self.reproducibility)
            ]
            answers = [
                await _output_parser.aparse(
                    result_text, p_value, self.llm, self.max_retries
                )
                for result_text in result_text_list
            ]
            answers = [answer.dict() for answer in answers if answer is not None]
            new_answer = {}
            for key in ["TP", "FP", "FN"]:
                key_answers = [
                    answer[key] for answer in answers if answer.get(key, []) != []
                ]
                if key_answers != []:
                    new_answer[key] = await ensembler.from_list_of_strings(key_answers)
                else:
                    new_answer[key] = []

            answers = AnswerCorrectnessClassification.parse_obj(new_answer)
        else:
